...
double local_dist(location_t& a, location_t& b) {
	return dist(a, b, tag);
}

void PMtree_t::__initDisk(Node_t *rt) {
	if (rt->isLeaf()) {
		// dump to File
		rt->pageId = pageIdx;
...
pair<int,int> PMtree_t::promotion(Node_t* p) {
	pair<int,int> ret;
...
void PMtree_t::balancedLeafPartition(Node_t* p, pair<int,int> promote, set<int>& setA, set<int>& setB) {
	setA.clear();
	setB.clear();
	int n = p->entry_N;
	...
	for (int i=0; i<n; ++i) {
		disA.push_back( make_pair(local_dist(V[promote.first], V[p->entry[i].first]), p->entry[i].first) );
		disB.push_back( make_pair(local_dist(V[promote.second], V[p->entry[i].first]), p->entry[i].first) );
	}
...
void PMtree_t::balancedNonLeafPartition(Node_t* p, pair<int,int> promote, set<int>& setA, set<int>& setB) {
	setA.clear();
	setB.clear();
	int n = p->child.size();
	vector<pair<double,int> > disA, disB;
	...
	for (auto iter=p->child.begin(); iter!=p->child.end(); ++iter) {
		disA.push_back( make_pair(local_dist(V[promote.first], V[iter->second->nomIdx]), iter->first) );
		disB.push_back( make_pair(local_dist(V[promote.second], V[iter->second->nomIdx]), iter->first) );
	}
	sort(disA.begin(), disA.end());
	sort(disB.begin(), disB.end());
...
Node_t* PMtree_t::splitLeaf(Node_t* p) {	
	...
	for (auto iter=setB.begin(); iter!=setB.end(); ++iter) {
		double disTmp = local_dist(V[*iter], V[promote.second]);
		q->addEntry(*iter, disTmp);
	}
	
	p->clearEntry();
	for (auto iter=setA.begin(); iter!=setA.end(); ++iter) {
		double disTmp = local_dist(V[*iter], V[promote.first]);
...
Node_t* PMtree_t::splitNonLeaf(Node_t* p) {	
	pair<int,int> promote = promotion(p);
	assert(promote.first == p->nomIdx); // confirmed split strategy
	...
	for (auto iter=setB.begin(); iter!=setB.end(); ++iter) {
		...
		son->far = q;
		son->dis2par = local_dist(V[son->nomIdx], V[q->nomIdx]);
		q->addChild(son);
		p->removeChild(son);
	}
...
void PMtree_t::insert(int pid) {
	if (rt == NULL) {
		...
		while (p!=NULL && !p->isLeaf()) {
			...
			for (auto iter=p->child.begin(); iter!=p->child.end(); ++iter) {
				double dis = local_dist(V[iter->second->nomIdx], V[pid]);
		...
		while (p != NULL) {
			double dis = local_dist(V[p->nomIdx], V[pid]);
			...
			if (p->isLeaf()) {
				...
				if (p->entry_N > maxLeafCapacity) {
					...
					if (p->far == NULL)
						...
						assert(dcmp(q->dis2par - local_dist(V[q->nomIdx], V[p->far->nomIdx])) == 0);
				}
			} else {
			...
			if (q != NULL) {
				if (p->far == NULL) {
					...
					p->far = q->far = rt;
					rt->addChild(p);
					q->dis2par = local_dist(V[rt->nomIdx], V[q->nomIdx]);
					rt->addChild(q);
				} else {
					// q->dis2par = local_dist(V[p->far->nomIdx], V[q->nomIdx]);
					p->far->addChild(q);
					q->far = p->far;
				}
			}
...
void PMtree_t::_searchLeafNode_RQ(double disToCenter, Node_t* p, location_t& loc, double radius, vector<int>& answers) {
	...
	for (int i=0; i<n_out; ++i) {
		if (fabs(disToCenter-dists[i]) <= radius) {
			if (local_dist(locs[i], loc) <= radius) {
...
void PMtree_t::_PMTree_RangeQuery(location_t& loc, double radius, vector<int>& answers) {
	...
	if (rt != NULL) {
		tmp.second = rt;
		tmp.first = local_dist(V[rt->nomIdx], loc);
		r = rt->radius;
		if (tmp.first<=r+radius && true==checkNodeBoundary(rt,radius))
			Q.push(tmp);
	}
		...
		if (false == p->isLeaf()) {// rt is an internal node
			for (auto iter=p->child.begin(); iter!=p->child.end(); ++iter) {
				...
				if (fabs(cur.first-q->dis2par) <= r+radius) {
					tmp.first = local_dist(V[pid], loc);
					if (tmp.first<=r+radius && true==checkNodeBoundary(q,radius)) {
						Q.push(tmp);
					}
				}
			}
...
void PMtree_t::_searchLeafNode_KnnQ(double disToCenter, Node_t* p, double& dk, location_t& loc, int kth, vector<int>& answers, set<nnpair_t, greater<nnpair_t> >& NNU) {
	nnpair_t nntmp;
	int NNU_sz = NNU.size();
	const int n_out = p->entry_N;
	const int pageId = p->pageId;
	...
	for (int i=0; i<p->entry_N; ++i) {
		...
		nntmp.first = local_dist(locs[i], loc);
		nntmp.second = -1 - pid;	
		if (nntmp.first <= dk) {
			NNU.insert(nntmp);
			++NNU_sz;
...
void PMtree_t::_PMTree_KnnQuery(location_t& loc, int kth, vector<int>& answers) { 
	...
	if (rt != NULL) {
		...
		tmp.second.first = local_dist(V[rt->nomIdx], loc);
		r = rt->radius;
	...
	while (!PQ.empty()) {
		...
		for (auto iter=p->child.begin(); iter!=p->child.end(); ++iter) {
			...
			pid = q->nomIdx;
			r = q->radius;
			if (fabs(cur.second.first-q->dis2par)<=r+dk && true==checkNodeBoundary(q,dk)) {
				tmp.second.first = local_dist(V[pid], loc);
				tmp.first = max(tmp.second.first-r, 0.0);
				if (tmp.first <= dk) {
					PQ.push(tmp);
