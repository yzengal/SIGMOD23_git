...
public:
	...
	public:
		...
		long long _memoryCost() {
			return sizeof(Data)+sizeof(double)*2;
		}
...
private:
	...
	public:
		...
		long long memoryCost() {
			long long ret = IndexItem::_memoryCost();
			// add the size of a map
			ret += (sizeof(Data) + sizeof(void*) + sizeof(_Rb_tree_node_base)) * children.size();
			for(typename ChildrenMap::const_iterator i = children.begin(); i != children.end(); ++i) {
					Node* child = dynamic_cast<Node*>(i->second);
					if (child != NULL)
						ret += child->memoryCost();
			}
			return ret;
		}
	...
	protected:
		...
		void updateMetrics(Node* child, double distance) {
			child->distanceToParent = distance;
			updateRadius(child);
		}
		
		void updateRadius(Node* child) {
			this->radius = std::max(this->radius, child->distanceToParent + child->radius);
		}
	...
	class LeafNodeTrait : public virtual Node {
		void doAddData(const Data& data, double distance, const mtree* mtree) {
			...
			entry->distanceToParent = distance;
			this->radius = std::max(this->radius, entry->distanceToParent + entry->radius);
		}
		...
		void addChild(IndexItem* newChild_, double distance, const mtree* mtree) {
			...
			while(!newChildren.empty()) {
				...
				if(i == this->children.end()) {
					...
					newChild->distanceToParent = distance;
					this->radius = std::max(this->radius, newChild->distanceToParent + newChild->radius);
				} else {
				...
				if(abs(distance - child->distanceToParent) <= child->radius) {
					...
					if(distanceToChild <= child->radius) {
						try {
							...
							this->radius = std::max(this->radius, expandedChild->distanceToParent + expandedChild->radius);
							return;
						}
					}
				}
	...
	protected:
		void updateMetrics(Entry* child, double distance) {
			child->distanceToParent = distance;
			updateRadius(child);
		}
		
		void updateRadius(Entry* child) {
			this->radius = std::max(this->radius, child->distanceToParent + child->radius);
		}
	};
};
