...
#define PIVOT_N 	5
#define SFC_nBits 	12
#define GRID_L 		((1ULL<<SFC_nBits)-1)
...
public:
    ...
    {
		unsigned long long coord[PIVOT_N];
		unsigned long long cornerlo[PIVOT_N];
		unsigned long long cornerhi[PIVOT_N];
		unsigned long long work[PIVOT_N];
        if (n->isleafnode())
        {
           leaf_node *leafnode = static_cast<leaf_node*>(n);
		   leafnode->key_min = numeric_limits<key_type>::max();
		   leafnode->key_max = numeric_limits<key_type>::min();
			for (int i=0; i<leafnode->slotuse; ++i) {
			...
			SFC_value = hilbert_c2i(PIVOT_N, SFC_nBits, cornerlo);
        ...
        {
            inner_node *innernode = static_cast<inner_node*>(n);
			innernode->key_min = numeric_limits<key_type>::max();
		    innernode->key_max = numeric_limits<key_type>::min();
            for (int slot = 0; slot < innernode->slotuse + 1; ++slot)
            {
