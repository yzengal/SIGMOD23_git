...
void readFromDisk(int pageID, location_t* V, double* dists, size_t n_out) {
	...
	for (size_t i=0; i<n_out; ++i) {
		nr += fread(V+i, sizeof(location_t), 1, f);
		nr += fread(dists+i, sizeof(double), 1, f);
	}
    assert(nr == 2*n_out || !"could not read whole file");
...
void readFromDisk(int pageID, location_t* V, unsigned long long* keys, size_t n_out) {
	...
	for (size_t i=0; i<n_out; ++i) {
		nr += fread(V+i, sizeof(location_t), 1, f);
		nr += fread(keys+i, sizeof(unsigned long long), 1, f);
	}
    assert(nr == 2*n_out || !"could not read whole file");
...
void dumpToDisk(int pageID, int* pids, unsigned long long* keys, size_t n_out) {
	...
	for (size_t i=0; i<n_out; ++i) {
		nr += fwrite(V+pids[i], sizeof(location_t), 1, f);
		nr += fwrite(keys+i, sizeof(unsigned long long), 1, f);
	}
    assert(nr == 2*n_out || !"could not write whole file");
...
void dumpToDisk(int pageID, pair<int, double>* entry, size_t n_out) {
	...
	for (size_t i=0; i<n_out; ++i) {
		nr += fwrite(V+entry[i].first, sizeof(location_t), 1, f);
		nr += fwrite(&entry[i].second, sizeof(double), 1, f);
	}
    assert(nr == 2*n_out || !"could not write whole file");
