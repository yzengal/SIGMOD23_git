...
int mem;
void traverse() {
	queue< pair <Node_bkt*, int> > q;
	q.push(make_pair(rt,-1));
	while (!q.empty()) {
		pair <Node_bkt*, int> tmp = q.front();
		q.pop();
		Node_bkt* u = tmp.first;
		int fa = tmp.second;
		mem += sizeof(int) + sizeof(int) + sizeof(bool);
		mem += sizeof(int) + u->bucket.size() * sizeof(int);
		mem += sizeof(int) + u->lower.size() * sizeof(double);
		mem += sizeof(int) + u->child.size() * sizeof(Node_bkt*);
		if (u->isleaf) {
			;
		} else {
