...
void build(Node_t *node, vector <int> &cand, int ary) {
    ...
    for (int i = 0; i < ary; ++ i) {
        Node_t *tt = new Node_t();
        node->child.push_back(tt);
    }
    ...
    for (int i = 0; i < cand.size(); ++ i) {
        ...
        if (node->pivot[nearchild].maxd < neardist) {
            node->pivot[nearchild].maxd = neardist;
        }
        if (node->pivot[nearchild].mind > neardist) {
            node->pivot[nearchild].mind = neardist;
        }
    }
    for (int i = 0; i < ary; ++ i) {
        node->child[i]->pivot.clear();
        node->child[i]->child.clear();
...
void _RangeQuery(Node_t *node, location_t &loc, double radius, vector<int> &answers) {
    ...
    for (int i = 0; i < node->child.size(); ++ i) {
        if (disloc[i] - radius < neardist + radius && disloc[i] - radius < node->pivot[i].maxd) {
...
void _KnnQuery(Node_t *node, location_t &loc, int kth, priority_queue<pair<double, int> > &q) {
    ...
    for (int i = 0; i < node->child.size(); ++ i) {
        ...
        if (q.size() < kth || disloc[i].first - q.top().first < node->pivot[disloc[i].second].maxd) {
